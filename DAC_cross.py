'''
coded by lch
consider single value function, double frozen
target value function are all low value function
current value function can be high value function
'''
...
class DACAgent():
    ...
    def __init__(self, config, lowtran, hightran, device):
        ...
        self.lownet_target = lowPolicy(config.get('feature_dim'), config.get('action_dim'), config.get('num_options'),
                                       config.get('hidden_dim')).double().to(self.device)
        self.highnet_target = OptionNet(config.get('num_options'), config.get('feature_dim'),
                                        config.get('hidden_dim')).double().to(self.device)

        for target_param, param in zip(self.lownet_target.parameters(), self.lownet.parameters()):
            target_param.data.copy_(param.data)
        for target_param, param in zip(self.highnet_target.parameters(), self.highnet.parameters()):
            target_param.data.copy_(param.data)
    ...
    def lowtrain(self):
        ...
        with torch.no_grad():
            ...
            value_high_next = self.highnet_target(s_)['value']
            option_change_next = torch.where(option > 5, torch.zeros_like(option), option)
            value_high_next_zeros = torch.gather(value_high_next, 1, option_change_next.long())
            value_high_next = torch.where(option > 5,
                                          value_high_next.sum(dim=1, keepdim=True) / self.config.get('num_options'),
                                          value_high_next_zeros)

            value_high_now = self.highnet_target(s)['value']
            option_change_now = torch.where(pre_option > 5, torch.zeros_like(pre_option), pre_option)
            value_high_now_zeros = torch.gather(value_high_now, 1, option_change_now.long())
            value_high_now = torch.where(pre_option > 5,
                                         value_high_now.sum(dim=1, keepdim=True) / self.config.get('num_options'),
                                         value_high_now_zeros)
            ...
            delta_high = r + (1 - done) * self.config.get('gamma') * value_high_next - value_high_now
            adv_high = torch.zeros_like(delta_high)
            adv_high[-1] = delta_high[-1]
            # GAE
            for i in reversed(range(buffer_capacity - 1)):
                adv_high[i] = delta_high[i] + self.config.get('tau') * (1 - done[i]) * adv_high[i + 1]

            target_v = value_high_now + adv_high
        ...
        for target_param, param in zip(self.lownet_target.parameters(), self.lownet.parameters()):
            target_param.data.copy_(target_param.data * (1.0 - self.config.get('soft_tau')) +
                                    param.data * self.config.get('soft_tau'))
        for target_param, param in zip(self.highnet_target.parameters(), self.highnet.parameters()):
            target_param.data.copy_(target_param.data * (1.0 - self.config.get('soft_tau')) +
                                    param.data * self.config.get('soft_tau'))
        ...
        for _ in range(self.config.get('ppoepoch')):
            for index in BatchSampler(SubsetRandomSampler(range(buffer_capacity)), batch_size, False):
                ...
                option_change_now = torch.where(pre_option[index] > 5, torch.zeros_like(pre_option[index]),
                                                pre_option[index])
                value_now_zeros = torch.gather(value_now, 1, option_change_now.long())
                value_high_now_short = torch.where(pre_option[index] > 5,
                                                   value_now.sum(dim=1, keepdim=True) / self.config.get('num_options'),
                                                   value_now_zeros)
